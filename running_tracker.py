...
# Dir input and output
VIDEO_INPUT = f"videos/input/{VIDEO_FILE}"
OUTPUT_DIR =  "videos/output/"

# Strides por Segundo (Passadas)
STOPPED_THRESH = 0.4
WALKING_THRESH = 1.0
TROTTING_THRESH = 1.8
RUNNING_THRESH = 5

# Análse angulação de Joelhos
WALKING_KNEE_ANGLE = 100
TROTTING_KNEE_ANGLE = 130
RUNNING_KNEE_ANGLE = 145
...
# Initialize stride data storage
stride_data = {}
prev_time = 0
...
def calculate_angle(quadril, joelho, calcanhar):
    angle1 = quadril - joelho
    angle2 = calcanhar - joelho
    cosine_angle = np.dot(angle1, angle2) / (np.linalg.norm(angle1) * np.linalg.norm(angle2))
...
while cap.isOpened():
    ...
    if results[0].boxes is not None and results[0].boxes.id is not None:
        ...
        for box, track_id, kpts in zip(boxes, track_ids, keypoints_list):
            ...
            # Calculate knee angle range for better classification
            knee_angles = []
            ...
            if left_knee_angle:
                cv2.putText(frame, f"L Knee: {left_knee_angle:.0f} degrees", 
                            (int(x1), angle_text_y), cv2.FONT_HERSHEY_SIMPLEX, 0.6, color, 1)
                angle_text_y += 20
            if right_knee_angle:
                cv2.putText(frame, f"R Knee: {right_knee_angle:.0f} degrees", 
                            (int(x1), angle_text_y), cv2.FONT_HERSHEY_SIMPLEX, 0.6, color, 1)
